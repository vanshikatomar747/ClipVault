...
import { updateNotebook, deleteNotebook } from '../api/notebooks';
...
export default function EditNotebookModal({ isOpen, onClose, notebook }: EditNotebookModalProps) {
  ...
  useEffect(() => {
    if (notebook && isOpen) {
      ...
      setColor(notebook.color || '#A8C3A0');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateNotebook(notebook._id, { name, description, color, icon });
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this notebook? All items inside will be lost.')) {
      setIsDeleting(true);
      try {
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm p-4">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-cv-brown">Edit Notebook</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="pt-4 flex items-center justify-between gap-3">
              <button
                ...
                disabled={isDeleting}
                className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors flex items-center gap-2 font-medium"
              >
